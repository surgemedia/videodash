<?php
	...
	$cca_row = mysql_fetch_array($check_client_active);
            $checksamelink = mysql_query("SELECT * FROM video_project WHERE id = ".$project_id." ORDER BY id DESC LIMIT 0,1");
            //echo "SELECT * FROM video_project WHERE id = ".$project_id." ORDER BY id DESC LIMIT 0,1<br/>";
	...
	if($_POST['Add_videos']==1 && $project_id!="" && $_POST['video_input']!=""){
		$check_total_version = mysql_query("SELECT * FROM video_under_project WHERE video_project_id =".$project_id);
		$check_total_version_num = mysql_num_rows($check_total_version);
        $check_video_Lastupdate = mysql_query("SELECT * FROM video_under_project WHERE  video_project_id =".$project_id." AND enabling = 1 ORDER BY id DESC LIMIT 0, 1");
		...
		if($check_video_Lastupdate_row['version']=="Final"){
            ...
            $query = mysql_query("INSERT INTO video_under_project VALUE(NULL, ".$project_id.", '".$_POST['video_input']."', '".$_POST['version']."', '".htmlspecialchars($_POST['notes'], ENT_QUOTES)."', 1, ".$videoversion_num.", NULL)");
        }
        ...
        if(!$query){
			echo "INSERT INTO video_under_project VALUE(NULL, ".$project_id.", '".$_POST['video_input']."', '".$_POST['version']."', '".htmlspecialchars($_POST['notes'], ENT_QUOTES)."', 1, ".$videoversion_num.", NULL)";
            ...
            if($_POST['downloadlink']!=""){
                ...
                if($_POST['downloadlink']!=$checksamelinkrow['download_file']){
                    //echo "UPDATE video_project SET download_file = ".$_POST['downloadlink']." WHERE id = ".$project_id;
                    mysql_query("UPDATE video_project SET download_file = '".$_POST['downloadlink']."' WHERE id = ".$project_id);
	...
	$check_video_Lastupdate = mysql_query("SELECT * FROM video_under_project WHERE  video_project_id =".$project_id." AND enabling = 1 ORDER BY id DESC LIMIT 0, 1");
	$check_video_Lastupdate_row = mysql_fetch_array($check_video_Lastupdate);
	$check_project_name = mysql_query("SELECT * FROM video_project WHERE  id =".$project_id);
                ...
                <ul  >
                    <li id="add_new_video" class="video_obj featured">
                     <!-- <a onClick="document.getElementById('back_to_project').submit();"><h1 class="back_button"><i class="fa  fa-reply"></i> All Projects</h1></a> -->
                    	...
                    	<form action="add_video2.php" method="post" id="add_video">
